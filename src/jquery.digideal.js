(function($) {
	...
	$.fn.DigiDeal = function (options,value) {
		
		var data = $(this).data('_DigiDeal');
		...
		if(data) {
			...
			var settings = $.extend({}, $.fn.DigiDeal.defaults, options);
			// these paramters are required

			if(this.selector.indexOf('#') === -1) {
				throw('DigiDeal can only be called with an ID, not a class or "naked" element');
			}
			$(this).empty();
			var dgp = new DigiDeal(settings,$(this));
